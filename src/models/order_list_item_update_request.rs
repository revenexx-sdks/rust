...
/// Partial update — omitted fields keep their current value. `quantity` is
/// still checked (> 0) and the article is still checked against the catalogue
/// where `reject_unknown_articles` is on.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct OrderListItemUpdateRequest {

...
/// An asset is bound to its bytes on creation: source 'storage' needs
/// `storage_asset_id`, source 'external' needs `external_url` AND an explicit
/// `source` — the column defaults to 'storage', so an `external_url` on its
/// own is refused by the database with a bare "a value is not allowed here".

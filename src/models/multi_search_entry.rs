...
/// One search inside a federated request. `collection_name` is required on the
/// gateway-trust path; with a `revx_` key it is optional and is forced to the
/// key's own collection.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MultiSearchEntry {
    /// A collection the tenant owns.
    #[serde(rename = "collection_name", default)]
